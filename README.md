...
*Minecraft `26.1.2` · NeoForge · 当前版本 `0.4.1-alpha`（α 开发阶段）*
...
**0.4.1（裂隙祭坛彻底重做 + 赛季管理命令）**：①裂隙祭坛第三版——打开的是**纯原版双箱界面**
（`GENERIC_9x3`）：不再使用任何自定义菜单类型或自定义屏幕，客户端**零注册**（前两版 UI 正是
死在这一点上），点击 / 拖拽 / Shift 快移 / 快速合成全部为原版行为；全部锻造规则移至服务端槽位
逻辑：第一排 1–3 格放同品质纯净卡 → 第 4 格取融合产物；第 5 格卡牌 + 第 6 格生态 Boss 材料 →
第 7 格取升星产物（25 物资点、5★ 上限、物资点不足时预览自动消失，取物与扣费在同一服务端
点击处理内原子完成）；Shift 点击自动按 融合→升星 顺序入槽；关闭时材料自动归还；打开时聊天栏
发送一行用法说明；②新增管理员/测试命令：`/eden season set <n>`（跳转第 n 个已注册赛季）、
`/eden season done <id|all>`（标记委托完成，喂 3/5 推进门）、`/eden season gate`（直接开启终局
之门并集合全员）、`/eden season rerun`（强制重开赛季多周目）。

**0.4.0（S1 矿洞季·批D 终局演出 + 赛季注册表）**：S0 矿洞季迎来收官之战——
①**赛季注册表抽象**：赛季成为可注册内容包（`SeasonDefinition`：委托/修饰层/残片/终局
规格），本体自带 S0（原 S1 更名），**后续赛季将以独立扩展模组形式发布**，依赖本体即可注册
新赛季，旧存档经 season_index 平滑迁移；②**虚空竞技场终局挑战**：≥3/5 委托 + 全服过半投票
通过后**终局之门永久开启**，全员传送至代码生成的深暗残响竞技场（磨制深板岩圆盘 + 幽匿
Boss 台 + 灵魂灯看台），15 秒倒计时后**深渊之形**（强化监守者，基础 800 生命、按参战人数
+25%/人，66%/33% 两段相位：低语织网者与掘穴者涌出、本体加速增伤）苏醒——败退不算失败，
回污染世界发育后随时再战；Boss 数值由 Eden 按人数缩放，战斗特效层由 entity_modifier
配置档承载（首次运行自动生成默认档，管理员可在修改器 GUI 里可视化调校）；③**终局仪式与
4 结局**：击败后三起草人各自陈述（引用你们实际找到的残页数）、AI 宣读三值读数、结局判定
（净土纪元/共生潮汐/存档长夜三领头值 + 隐藏结局**三重奏**：18 页全收集且三值均 ≥60）、
结局视频槽（可选资产）、焦黑缺页特写收尾——三角符文微微发光，为下季留种；④**净化收官 +
多周目**：Boss 陨落即完成赛季净化条件，污染经正常战役管线归零（阶段奖励/胜利广播/天堂门
全量触发），编年史墙新增结局图鉴与「重开赛季」投票（战役重置、元进度与图鉴保留）。
...
**接下来**：v2 余项（卡牌协同套装、世界复净演出、编年史高光外显）；**S1 起的后续赛季将以独立
扩展模组发布**（本体已就位赛季注册表 API）。浊潮生态首批（污染积液 / 生态巢穴 Boss ×3 / 浊雨 /
裂隙祭坛锻造）已实装。欢迎在 Issues 里提想法。
