...
*Minecraft `26.1.2` · NeoForge · 当前版本 `0.3.5-alpha`（α 开发阶段）*
...
**0.3.5（关键修复批次）**：①裂隙祭坛右键无反应——容器菜单此前**从未在客户端注册屏幕**，服务端开屏包
到达后 `MenuScreens` 找不到工厂只留下一条警告；现经 `RegisterMenuScreensEvent` 注册
`RiftAltarScreen`，右键即开（点击/拖拽/Shift 移动等原版槽位交互全部随菜单生效）；②「修改器」点击
战利品无反应——枚举战利品表时误用 `registryAccess()`，而战利品表位于**可重载注册表层**（RELOADABLE），
查询必然抛异常导致回包永不发出；改经 `server.reloadableRegistries()` 枚举，列表按 id 排序返回；
③同类隐患一并修复：拾荒职业主动「发现」（R 键）使用了同一条坏注册表路径，按 R 即服务端报错，
同样迁移到正确路径。
